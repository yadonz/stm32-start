...
/**
  * @brief 这是 TIM3 中断函数，通过 TIM3 对PA1 引脚推挽输出写入 0 1 来输出一个周期为 1 的方波信号
  */ 
void TIM3_IRQHandler(void)
{
	if(TIM_GetITStatus(TIM3, TIM_IT_Update) == SET)		// 进中断前要检查中断标志位
	{
		GPIO_WriteBit(GPIOA, GPIO_Pin_1, (BitAction)!GPIO_ReadOutputDataBit(GPIOA, GPIO_Pin_1));
		TIM_ClearITPendingBit(TIM3, TIM_IT_Update);		// 退出中断前一定要清除中断标志位才可以退出中断（否则会卡在中断中）
	}
}
